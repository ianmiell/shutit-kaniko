...
/swapfile none            swap    sw              0       0" >> /etc/fstab''')
		...
		for machine in sorted(machines.keys()):
			...
			shutit_session.send('mkdir -p /root/go/{src,bin}')
			shutit_session.send('export GOPATH=/root/go')
			shutit_session.send('export PATH=${PATH}:/usr/lib/go-1.10/bin/')
			shutit_session.install('libostree-dev')
			shutit_session.install('libgpme-dev')
