...
class shutit_kaniko(ShutItModule):
...
then
	virsh list | grep ${MODULE_NAME} | awk '{print $1}' | xargs $XARGS_FLAG -n1 virsh destroy
		...
		shutit.build['module_name'] = 'shutit_kaniko_' + ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(6))
...
/swapfile none            swap    sw              0       0" >> /etc/fstab''')
		...
		for machine in sorted(machines.keys()):
			...
			shutit_session.install('build-essential')
			shutit_session.install('pkgconfig')
			shutit_session.install('libgpgme11-dev')
			shutit_session.install('libglib2.0-dev')
			shutit_session.install('libsoup2.4')
			shutit_session.install('libselinux-dev')
			shutit_session.install('libdevmapper-dev')
			shutit_session.install('libarchive13')
			shutit_session.send('wget -r --no-parent http://packages.linuxmint.com/pool/import/o/ostree/')
			shutit_session.send('cd packages.linuxmint.com/pool/import/o/ostree/')
			shutit_session.send('dpkg -i libostree-1-1_2017.12-0alexlarsson1~xenial_amd64.deb ')
			shutit_session.send('dpkg -i gir1.2-ostree-1.0_2017.12-0alexlarsson1~xenial_amd64.deb ')
			shutit_session.send('dpkg -i libostree-dev_2017.12-0alexlarsson1~xenial_amd64.deb ')
			shutit_session.send('dpkg -i ostree_2017.12-0alexlarsson1~xenial_amd64.deb ')
			shutit_session.send('wget https://dl.google.com/go/go1.10.1.linux-amd64.tar.gz')
			shutit_session.send('tar -zxvf go1.10.1.linux-amd64.tar.gz')
			shutit_session.send('tar -C /usr/local -xzf go1.10.1.linux-amd64.tar.gz')
			shutit_session.send('export GOPATH=/usr/local/go/bin')
			shutit_session.send('export PATH=$PATH:/usr/local/go/bin')
...
def module():
	return shutit_kaniko(
		'git.shutit_kaniko.shutit_kaniko', 1592314804.0001,
		description='',
		maintainer='',
